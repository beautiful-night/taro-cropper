...
import Taro, { getSystemInfoSync as _getSystemInfoSync, getImageInfo as _getImageInfo, createCanvasContext as _createCanvasContext, canvasToTempFilePath as _canvasToTempFilePath } from "@tarojs/taro-h5";
...
class TaroCropperComponent extends Taro.PureComponent {
  constructor(props) {
    super(props);
    this.imageLeft = 0;
    this.imageTop = 0;
    ...
    this.systemInfo = _getSystemInfoSync();
    this.state = {
      scale: 1
    };
  }
  ...
  componentDidMount() {
    ...
    this.cropperCanvasContext = _createCanvasContext(cropperCanvasId, this);
    this.cropperCutCanvasContext = _createCanvasContext(cropperCutCanvasId, this);
    this.updateInfo(this.props).then(() => {
      this.update();
    }).catch(() => {
      this.update();
    });
  ...
  _drawCropperCorner() {
    ...
    this.cropperCanvasContext.moveTo(cropperStartX, cropperStartY - 1);
    ...
    this.cropperCanvasContext.moveTo(cropperStartX + this.cropperWidth, cropperStartY - 1);
    ...
    this.cropperCanvasContext.moveTo(cropperStartX, cropperStartY + this.cropperHeight + 1);
    ...
    this.cropperCanvasContext.moveTo(cropperStartX + this.cropperWidth, cropperStartY + this.cropperHeight + 1);
  ...
  _outsideBound(imageLeft, imageTop) {
    this.imageLeft = imageLeft > (this.width - this.cropperWidth) / 2 ? (this.width - this.cropperWidth) / 2 : imageLeft + this.scaleImageWidth >= (this.width + this.cropperWidth) / 2 ? imageLeft : (this.width + this.cropperWidth) / 2 - this.scaleImageWidth;
    this.imageTop = imageTop > (this.height - this.cropperHeight) / 2 ? (this.height - this.cropperHeight) / 2 : imageTop + this.scaleImageHeight >= (this.height + this.cropperHeight) / 2 ? imageTop : (this.height + this.cropperHeight) / 2 - this.scaleImageHeight;
  }
  ...
  render() {
    const { width, height, cropperCanvasId, fullScreen, fullScreenCss, themeColor, hideFinishText, cropperWidth, cropperHeight, cropperCutCanvasId, hideCancelText, onCancel, finishText, cancelText } = this.props;
    ...
    if (!hideFinishText) {
      ...
      // if (!isH5) {
      finish = <CoverView style={finishStyle} onClick={onFinishClick}>
        {finishText}
      </CoverView>;
      // } else {
      //   finish = <View
    ...
    if (!hideCancelText) {
      ...
      cancel = <CoverView style={cancelStyle} onClick={onCancel}>
        {cancelText}
      </CoverView>;
    }
    return <View className={`taro-cropper ${isFullScreenCss ? 'taro-cropper-fullscreen' : ''}`} style={croperStyle}>
        <Canvas canvasId={cropperCutCanvasId} style={cutCanvasStyle} className={`cut-canvas-item ${isFullScreenCss ? 'cut-canvas-fullscreen' : ''}`} ref={ref => {
        this['__taroref_cropperCutCanvasId'] = ref;
      }} />
        <Canvas onTouchStart={this.handleOnTouchStart} onTouchMove={this.handleOnTouchMove} onTouchEnd={this.handleOnTouchEnd} canvasId={cropperCanvasId} style={canvasStyle} className={`canvas-item ${isFullScreenCss ? 'canvas-fullscreen' : ''}`} disableScroll ref={ref => {
        this['__taroref_cropperCanvasId'] = ref;
      }}>
        </Canvas>
